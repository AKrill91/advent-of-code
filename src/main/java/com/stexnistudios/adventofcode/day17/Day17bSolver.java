...
public class Day17bSolver extends Solver {

    private final MessageDigest digest;

    public Day17bSolver(String input) throws Exception {
        super(input);
    ...
    public Integer call() {
        ...
        final AtomicInteger longestPath = new AtomicInteger(0);

        while (currentSteps.size() > 0) {
            Map<String, Point> nextSteps = new HashMap<>();

            currentSteps.entrySet()
                .forEach(entry -> {
                    String directions = entry.getKey();
                    Point pos = entry.getValue();

                    if (pos.equals(end)) {
                        if (directions.length() > longestPath.get()) {
                            longestPath.set(directions.length());
                        }
                        return;
                    }
